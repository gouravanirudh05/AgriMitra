...
def query_with_gemini(query: str, index, chunks, embedding_model, top_k=5):
    """Query the vector store and get answer from Gemini"""
    model = SentenceTransformer(embedding_model)
    query_embedding = model.encode([query])
    D, I = index.search(query_embedding, k=top_k)
    matched_chunks = [chunks[i] for i in I[0] if i < len(chunks)]
    
    print(f"\nTop {len(matched_chunks)} Retrieved Chunks:")
    for i, chunk in enumerate(matched_chunks):
        similarity_score = D[0][i]
        print(f"\n--- Chunk {i + 1} (Similarity: {similarity_score:.4f}) ---")
    ...
    context = "\n\n".join(matched_chunks[:3])  # Use top 3 chunks
    
    prompt = f"""Answer the following question using the provided context from various document types (PDFs, CSVs, Excel, Markdown, DOCX, JSON, Text files):
...
- Mention the source file types and names when relevant
- If the information comes from tabular data (CSV/Excel), highlight key statistics or findings
- Keep the answer focused and practical
- If information is insufficient, mention what additional details would be helpful
